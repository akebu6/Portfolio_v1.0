...
const projects = computed(() => 
  data.value?.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()))
</script>
...
<template>
  <section id="project" class="relative flex flex-col gap-8">
    ...
    <div
      class="relative left-1/2 -translate-x-1/2 flex flex-wrap gap-8 py-4 px-4 md:px-8 lg:px-10 xl:px-12 w-screen max-w-[85rem] overflow-hidden">
      <ProjectCard
