...
function onHandleSlider(dir: 'left' | 'right') {
  ...
  <section id="certificate" class="relative left-1/2 -translate-x-1/2 flex flex-col gap-8 w-screen overflow-x-hidden">
    ...
    <div class="relative flex justify-center py-4">
      <template v-for="({ name, link, date }, index) in certificates" :key="name">
        <CertificateCard v-if="index >= activeIndex - 1 && index <= activeIndex + 1" :name="name" :link="link" :date="date" :active="index === activeIndex"
        @slide="onHandleSlider"
